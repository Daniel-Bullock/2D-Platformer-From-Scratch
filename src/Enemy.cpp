...
Enemy::Enemy(float x_, float y_, float speed, int tileSize) {
    ...
    /*
    if(!texture.loadFromFile("Textures/Player/slimetest1.png")){
        return;
    }*/
...
void Enemy::update(std::vector<std::vector<int>>& tileMap, int tileSize) {
    // Check for collisions with walls and edges respectively (1 = right)
    //std::cout<<(int)(y / tileSize)<< ' '<<(int)((x + sprite.getGlobalBounds().width) / tileSize) <<' '<< (int)((y + sprite.getGlobalBounds().height) / tileSize) <<"\n";
    ...
    /*if ((direction == 1 && (tileMap[(int)(y / tileSize)][(int)((x + sprite.getGlobalBounds().width) / tileSize)] == 1 || 
                            tileMap[(int)((y + sprite.getGlobalBounds().height) / tileSize)][(int)((x + sprite.getGlobalBounds().width) / tileSize)] == 0)) ||
        (direction == -1 && (tileMap[(int)(y / tileSize)][(int)((x) / tileSize)] == 1 ||
                                tileMap[(int)((y + sprite.getGlobalBounds().height) / tileSize)][(int)((x) / tileSize)] == 0))) {
        // Change direction if the enemy hits a wall or edge
        direction *= -1;
    }*/
    //std::cout<<"here5\n";
    ...
    //std::cout<<movementSpeed*direction<<"\n";
}
